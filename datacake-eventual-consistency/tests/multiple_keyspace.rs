...
async fn test_single_node() -> anyhow::Result<()> {
    ...
    handle
        .get(KEYSPACE_1, key(1))
        .await
        .expect("Get doc.")
        .expect("Get document just stored.");
    let doc = handle.get(KEYSPACE_2, key(1)).await.expect("Get doc.");
    assert!(doc.is_none());
    let doc = handle.get(KEYSPACE_3, key(1)).await.expect("Get doc.");
    assert!(doc.is_none());

    handle
        .del(KEYSPACE_1, key(1), Consistency::All)
        .await
        .expect("Put doc.");

    let doc = handle.get(KEYSPACE_1, key(1)).await.expect("Get doc.");
    assert!(doc.is_none());
    let doc = handle.get(KEYSPACE_2, key(1)).await.expect("Get doc.");
    assert!(doc.is_none());
    let doc = handle.get(KEYSPACE_3, key(1)).await.expect("Get doc.");
...
async fn test_multi_node() -> anyhow::Result<()> {
    ...
    node_2_handle
        ...
        .expect("Get doc.")
        .expect("Get document just stored.");
    let doc = node_3_handle.get(KEYSPACE_2, key(1)).await.expect("Get doc.");
    assert!(doc.is_none());
    let doc = node_1_handle.get(KEYSPACE_3, key(1)).await.expect("Get doc.");
    assert!(doc.is_none());
    ...
    let doc = node_2_handle.get(KEYSPACE_1, key(1)).await.expect("Get doc.");
    assert!(doc.is_none());
    let doc = node_3_handle.get(KEYSPACE_2, key(1)).await.expect("Get doc.");
    assert!(doc.is_none());
    let doc = node_3_handle.get(KEYSPACE_3, key(1)).await.expect("Get doc.");
    assert!(doc.is_none());
...
async fn test_iter_metadata() -> anyhow::Result<()> {
    ...
    let keys: Vec<Vec<u8>> = handle
        ...
        .map(|entry| entry.0)
        .collect();
    let result: Vec<Vec<u8>> = vec![key(1), key(2), key(3)];
    ...
    let keys: Vec<Vec<u8>> = handle
        ...
        .map(|entry| entry.0)
        .collect();
    let result: Vec<Vec<u8>> = vec![key(100), key(99)];
    assert!(keys.iter().all(|item| result.contains(item)));
